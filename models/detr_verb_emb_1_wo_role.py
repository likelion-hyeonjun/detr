...
class DETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_roles = 190, num_verbs = 1, aux_loss=False):
        ...
        self.query_embed = nn.Embedding(self.num_verbs, hidden_dim) # only one query encoding for verb
    ...
    def forward(self, samples: NestedTensor):
        ...
        outputs_verb = self.verb_classification(hs)
...
class SWiGCriterion(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        return {'loss_vce': verb_loss, 'loss_nce': torch.tensor(0).cuda(), 'verb_error': verb_acc, 'noun_error': torch.tensor(0).cuda(), 'class_error': torch.tensor(0).cuda()}
